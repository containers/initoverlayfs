...
sudo clang -DUNLOCK_OVERLAYDIR=\"$UNLOCK_OVERLAYDIR\" -O3 -pedantic -Wall -Wextra -Werror pre-initoverlayfs.c -o $DIR_TO_DUMP_INITRAMFS/usr/sbin/pre-initoverlayfs
sudo gcc -DUNLOCK_OVERLAYDIR=\"$UNLOCK_OVERLAYDIR\" -O3 -pedantic -Wall -Wextra -Werror -fanalyzer pre-initoverlayfs.c -o $DIR_TO_DUMP_INITRAMFS/usr/sbin/pre-initoverlayfs
sudo valgrind $DIR_TO_DUMP_INITRAMFS/usr/sbin/pre-initoverlayfs
